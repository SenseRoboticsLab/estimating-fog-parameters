    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    {
        mThDepth = mbf*(float)fSettings["ThDepth"]/fx;      // 设置此深度阈值为 b*f*ThDepth/f = ThDepth*b,即ThDepth倍的基线长度
...
void Tracking::SetFogEstimator(FogEstimation* pFogEstimator)
{
    mpFogEstimator = pFogEstimator;
}
...
//    /* ---> original code */
    ...
    if (imRectLeft.channels() == 3)     // if the input image is actually colour
    {
        cvtColor(imRectLeft, mImGray,CV_BGR2GRAY);
        mCurrentFrame = Frame(imRectLeft,imRectRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth, true);
    }
    else
    {
        mImGray = imRectLeft;
    ...
    while (mpFogEstimator->GetBusy())   // wait until the fog estimation thread becomes free
    {
        usleep(3000);
    }
    Track();
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)    // 若 前一帧的速度为零 或 最近两帧内进行过重定位
                {
        ...
        if(bOK)
        {
            ...
            if(!mLastFrame.mTcw.empty())
            {
                ...
                mVelocity = mCurrentFrame.mTcw*LastTwc;     // 更新速度
            }
            else
                mVelocity = cv::Mat();
...
void Tracking::StereoInitialization()
{
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            ...
            if(z>0)
            {
                cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);     // 根据深度反投影成3D点
                MapPoint* pNewMP = new MapPoint(x3D,pKFini,mpMap);  // 创建地图点
                pNewMP->AddObservation(pKFini,i);           // 地图点添加关键帧
                pKFini->AddMapPoint(pNewMP,i);              // 关键帧添加对地图点的观测
                pNewMP->ComputeDistinctiveDescriptors();               // 计算地图点的描述子
                pNewMP->UpdateNormalAndDepth();                     // 更新平均观测距离和方向
...
void Tracking::MonocularInitialization()
{
    ...
    // step1. 若单目初始化器还没创建(即参考帧都尚未建立),则创建初始化器
    if(!mpInitializer)
    {
        // Set Reference Frame
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        // step2.4. 深度太大(深度越大位置越不准确)且临时地图点数目足够(多于100个),就停止生成临时地图点
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // step6. 匹配的地图点数超过10个就认为是跟踪成功，即假设的恒速运动模型成立
    return nmatchesMap>=10;
}
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            if(!mCurrentFrame.mvbOutlier[i])
            {
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound(); // 实际观测到该地图点的帧数加1;位姿估计用到该地图点
                if(!mbOnlyTracking)
                {
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mCurrentFrame.mvpMapPoints.begin(), vend=mCurrentFrame.mvpMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(pMP)
        {
            ...
            {
                pMP->IncreaseVisible(); // 理论上应当观测到该地图点的帧数加1（这些点未必真的被当前帧观测到了,只是地理位置上处于当前帧视锥范围内）
                pMP->mnLastFrameSeen = mCurrentFrame.mnId;
                pMP->mbTrackInView = false;
            }
        }
